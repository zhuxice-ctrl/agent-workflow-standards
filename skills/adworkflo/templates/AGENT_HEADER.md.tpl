...
9. Default to one worker for a single task. For TODOwork batches, worker count is unbounded by design and follows TODO module split plus ARCH dependencies.
10. After edits, update `.adworkflow/worker_state.json`.
11. Before claiming completion, update `.adworkflow/verification_result.json`.
...
10. Update `worker_state.json`, `verification_result.json`, and `review_findings.json` when risk requires review.
